...
### v1.1.3(user 看过 v1.1.2 之后)

user 提的三处:健康检查只剩一个维度(深挖里 --include 的局部运行被存成最新,现在不存档);
Top SQL 另三个维度灰着点不动(改为可点,给「还没取过」+ 取数入口;逻辑读写明脚本不返回数值);
「调优 →」与「在会话里调优这条」两种说法并存、S2 已调优其他未调优看着不一致(统一成一种写法;
只有 S2 是已调优,是因为测试里模型只调优了一条,不是页面问题)。
