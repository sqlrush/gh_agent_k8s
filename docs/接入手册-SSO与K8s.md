...
| 项 | 谁做 | 说明 |
|---|---|---|
| 四个镜像与 K8s 清单 | **我方** | 离线包,含 amd64 与 aarch64 |
...
> **2026-09-21 状态策略变更**：Pod 启动时把状态从 NAS 拷到本地盘，读写在本地，
> 每 60 秒回写，销毁前完整回写。这是为了绕开 SQLite 跑在 NFS 上的性能瓶颈。
> **代价见 §7.3，贵方已知悉并接受。**
...
贵方 2026-09-20 提出：密钥审批周期紧张，**先只加 Appkey 与 Timestamp**。已支持：
...
贵方 2026-09-20 确认要显式收工号。字段名与位置可配（两处都做成开关是因为贵方尚未给出口径）：
